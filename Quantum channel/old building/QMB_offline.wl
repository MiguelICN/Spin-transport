...
TranslationEigenvectorRepresentatives::usage = "TranslationEigenvectorRepresentatives[L] returns a list of sublists, each containing:\
...
BlockDiagonalize::usage = "BlockDiagonalize[matrix,opts] returns ```matrix``` in block-diagonal form. Only \
	option is '''Symmetry'''."


Symmetry::usage = "Symmetry is an option for '''BlockDiagonalize''' to specify the symmetry according to which \
	```matrix``` in '''BlockDiagonalize'''[```matrix```] is block diagonalized. It takes the values: \
	\"Translation\". Default option is \"Translation\"."


Transform::usage = "Matrix transform for Translation symmetry"
...
IsingHamiltonian::usage = "IsingHamiltonian[h_x,h_z,J,L,opts] returns the Hamiltonian \ 
...
BoundaryConditions::usage = "BoundaryConditions is an option for '''IsingHamiltonian''' to specify the boundary conditions. It \
