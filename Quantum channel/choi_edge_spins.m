...
(**)
(*(* S = Q1 x Q2, dS = 4 (fixed: two spin-1/2 edge spins).           *)*)
(*(* dE = 2^L is set at the start of each L iteration.               *)*)
(*dS = 4;*)
(**)
...
(*(* ================================================================ *)*)
(*Do[*)
(*  (* Force integer L: Do iterates over Range[6,8] which gives integers, *)*)
(*  (* but guard explicitly so any accidental Real coercion is caught.     *)*)
(*  LL = IntegerPart[L];*)
(**)
...
(*    Clear[HT];*)
(**)
(*    (* dE: chain Hilbert space dimension *)*)
(*    dE = 2^LL;*)
(**)
(*    (* Sanity: eigenvector matrix must be D_total x D_total = (4*dE) x (4*dE) *)*)
(*    Print["      Evec dimensions: ", Dimensions[Evec],*)
(*          "  expected: {", 4*dE, ", ", 4*dE, "}"];*)
(**)
(*    (* Permute eigenvectors: [Q1,Chain,Q2] -> [Q1,Q2,Chain] = S x E *)*)
(*    EvecSE = PermuteEvecsToSE[Evec, LL];*)
(*    Clear[Evec];*)
(**)
(*    (* SpectralUnitary: beta-independent, built once per (LL, delta)  *)*)
...
(*      t0 = AbsoluteTime[];*)
(**)
(*      (* Environment initial state: dE x dE thermal chain state *)*)
...
(*      Clear[rhoChain];*)
(**)
(*      (* Sanity check: choiMat(0) = |Phi+><Phi+| => Tr[choiMat^2] = 1 *)*)
(*      choi0 = choiFn[0.];*)
...
(*      Clear[choiFn];*)
(**)
(*      MakePlotsChoi[choiResults, LL, k, beta, dS, dE, dirChoi];*)
(*      Clear[choiResults];*)
(**)
